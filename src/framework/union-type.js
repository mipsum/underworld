...
function rawCase(type, cases, value) {
  var handler = cases[value._name];

  if (handler === undefined) {
    handler = cases['_']; // default case
  }
  ...
  if (handler !== undefined) {
    return handler.apply(undefined, valueToArray(value));
  }
}
...
Type.ListOf = function (T) {
  ...
  var validate = List.case({
    List: function (array) {
      ...
      // the goal here is avoiding try-catch in prod.
